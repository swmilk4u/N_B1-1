...
- **웹페이지 시각화**: https://swmilk4u.github.io/N_B1-1/
...
```
> [!NOTE]
> **설계 해설 (Zero-shot 입력을 유도하는 Few-shot 작동 원리)**
> * 사용자는 아무런 사전 맥락 없이 단 1줄의 모호한 질문(Zero-shot 형태의 입력)을 던졌지만, 시스템 프롬프트에 내장된 Few-shot(예시 3)의 입출력 가이드 쌍 덕분에 AI가 스스로 단정 지어 오답을 날조하지 않고 안전한 확인 질문(역질문)을 던져 최종 Pass 판정을 유도하도록 구현된 핵심 설계입니다.
...
| 단계 | 발화 주체 | 시나리오 주요 내용 | 검증 목적 및 관련 시스템 프롬프트 설계 규칙 |
| :---: | :---: | :--- | :--- |
