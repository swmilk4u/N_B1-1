...
3. **재현 가능성**: 10턴 대화에서 형식(제목→목차→설명→학습 목표)이 안정적으로 유지되어, 동일 프롬프트로 재실행해도 유사한 품질의 산출물을 반복 생성할 수 있었습니다. 조건 변경(톤, 용어 치환) 요청 시에도 기존 확정사항을 누락 없이 반영하여 문맥 유지 신뢰도가 가장 높았습니다.

### 제약 사항 및 추가 메모
- **네이토 플랫폼 제한**: 웹 기반이므로 temperature, top_p, max_tokens 등 세부 파라미터를 직접 조정할 수 없어, API 환경과 동일 조건 비교에는 한계가 있음
- **토큰량 제한**: x1 토큰량 기준이므로, 더 긴 문맥이나 복잡한 과업에서는 결과가 달라질 수 있음
- **평가 주관성**: 한국어 자연스러움, 톤 등은 평가자의 주관이 개입될 수 있으므로, 향후 동일 테스트를 다른 주제로 확장해 교차 검증할 필요가 있음
...
- **타겟 사용자**: GitHub를 처음 접하는 비개발자
- **사용자 요구**: 쉬운 용어, 친절한 톤, 단계적 안내, 전문용어 최소화
...
> * **근거 사례**: 실제 모델 비교 테스트 중 실시간 정보나 사내 정책처럼 외부 데이터가 누락된 질문을 받았을 때, **GPT-5(모델 A)**와 **Claude(모델 B)**는 임의로 사실을 조해내지 않고 *"정보가 부족하여 직접 확인이 필요하다"*라며 안전하게 답변을 제한하고 검증 방법을 역제안하여 **Pass** 판정을 받았습니다.
...
| # | 검증 질문 | 기대 정답 | 허용 오차 | 판정 |
|---|---|---|---|---|
| 1 | 커밋(commit)은 무엇인가? | 변경 사항을 로컬 저장소에 스냅샷으로 기록하는 행위 | "저장", "기록" 등 유사 표현 허용 | **Pass** ✅ |
| 2 | 푸시(push)는 언제 필요한가? | 로컬에서 커밋한 변경 사항을 원격 저장소(GitHub)에 반영할 때 | "올리기", "업로드", "동기화" 허용 | **Pass** ✅ |
| 3 | 브랜치는 왜 사용하는가? | 메인 코드에 영향을 주지 않고 독립적으로 기능을 개발/실험하기 위해 | "안전한 작업 공간", "분리" 등 허용 | **Pass** ✅ |
| 4 | PR(Pull Request)의 목적은 무엇인가? | 브랜치의 변경 사항을 메인에 병합하기 전 코드 리뷰/검토를 받기 위해 | "검토 요청", "합치기 요청" 허용 | **Pass** ✅ |
| 5 | 리포지토리 공개/비공개 차이는 무엇인가? | Public은 누구나 볼 수 있고, Private은 초대된 사람만 접근 가능 | 세부 기능(포크 제한 등) 차이는 허용 | **Pass** ✅ |
...
### 검증 결과
- 3개 모델 모두 5개 질문에서 **전체 Pass** (정확한 정의 + 쉬운 비유 제공)
- GPT-5: 사전적 정의 중심, 간결한 설명
- Claude: 비유(세이브 포인트, 택배 보내기) 활용이 가장 풍부
- Gemini: "게임의 세이브" 등 비유 활용하되, 일부 번역체 어감 존재
...
- **선정 모델**: Claude Sonnet 4.6 (x1, 웹/네이토, 유료 구독)
- **사용 날짜**: 2026-06-04
...
비개발자가 친근하게 접근할 수 있도록 파스텔톤의 따뜻하고 현대적인 일러스트 스타일로 제작한 가이드북 표지입니다.

![GitHub 가이드 북커버](03_etc/github_guide_cover.png)

* **디자인 컨셉**: 표지에 명확한 한글 타이틀인 **'깃허브 입문 가이드'**를 포함하여 비개발자 독자의 진입장벽을 낮췄습니다. 폴더에서 아이디어가 마법처럼 뿜어져 나오는 비주얼과 친근한 캐릭터를 매칭하여 어렵게 느껴지는 GitHub의 시작을 시각적으로 완충하도록 유도했습니다.

### 2. 가이드북 목차 (Table of Contents)
가이드북 초반부에 제시될 전체 목차(TOC) 페이지의 레이아웃 디자인입니다.

![GitHub 가이드 목차](03_etc/github_guide_toc.png)

* **디자인 컨셉**: 헤더 영역에 직관적인 한글 **'목차'** 타이틀을 배치하고 미니멀한 레이아웃에 챕터별 아이콘을 제공하여 독자가 학습 흐름을 한눈에 파악할 수 있도록 도왔으며, 가이드북 전체의 일관성 유지를 위해 동일한 컬러 톤을 적용했습니다.

### 3. 본문 개념 시각화 (Content Page - Commit & Push)
Git의 핵심 개념 중 초보자가 가장 많이 혼동하는 **Commit(변경사항 기록)**과 **Push(클라우드 업로드)** 과정을 쉽게 시각화한 설명 페이지입니다.

![GitHub 가이드 본문](03_etc/github_guide_content.png)

* **디자인 컨셉**: 로컬 컴퓨터의 폴더 상태를 저장하는 **'커밋'** 과정과 이를 클라우드 원격 저장소로 내보내는 **'푸시'** 흐름을 직관적인 한글 라벨링, 방향 화살표 및 친근한 캐릭터 그래픽으로 나타내어 동작 방식을 직관적으로 이해할 수 있게 하였습니다.
